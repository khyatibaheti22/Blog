...
function senitizeForm($postedData){
	foreach($postedData as $k=>$val){
		if(!is_array($val)){
			$postedData[$k]=strip_tags($val);		
        }
        else{
            foreach($val as $key=>$value){
                if(!is_array($value)){
                    $postedData[$k][$key] = strip_tags($value);
                }
            }
        }
	}
...
function getUserImage($user_image,$type=false){
	...
	if($user_image!="" and file_exists(storage_path('app/public/avtar/'.$user_image))){
		...
		$image_url = "bx_user-circle.png";
	}
	switch($full_url){		
		default: return  $image_url = $httpPath.'/'.$image_url; break;
	}
}
// function getImage($user_image,$type=false){
//     $httpPath = asset('assets/storage/master-resume/');

// 	if($user_image!="" and file_exists(storage_path('app/public/master-resume/'.$user_image))){
//         if($type=='thumb'){
//             $image_url = str_replace('-', 'thumb_-', $user_image);
//         }
//         else{
